...
namespace StudentAffairs.Services.SecondLanguage
{
    ...
    {
        public List<Models.SecondLanguage> GetAll()
        {
            using (var dbContext = new StudentAffairsEntities())
            {
                var model = dbContext.SecondLanguages.Where(x => x.IsDeleted == false).OrderBy(x => x.CreatedOn).ToList();
                return model;
            }
        }
        ...
        public ResultDto<Models.SecondLanguage> Create(Models.SecondLanguage model, Guid UserId)
        {
            using (var dbContext = new StudentAffairsEntities())
            {
                ...
                var Oldmodel = dbContext.SecondLanguages.Where(x => x.Name == model.Name && x.IsDeleted == false).FirstOrDefault();
                if (Oldmodel != null)
                {
