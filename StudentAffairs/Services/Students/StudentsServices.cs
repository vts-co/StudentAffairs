...
namespace StudentAffairs.Services.Students
{
    public class StudentsServices
    {
        public List<StudentsDto> GetAll(Guid UserId, Guid EmployeeId, Role RoleId)
        {
            ...
            {
                var model = dbContext.Students.Where(x => x.IsDeleted == false&&x.StudentTypeId== (int)StudentTypes.InSchool && (x.CreatedBy == UserId || RoleId == Role.SystemAdmin)).OrderBy(x => x.CreatedOn).Select(x => new StudentsDto
                {
                    //بيانات الطالب
                    Id = x.Id,
                    Code = x.Code,
                    ...
                    IM = x.IM,
                    SecondLanguageIId = x.SecondLanguageIId != null ? x.SecondLanguageIId : Guid.Empty,
                    SecondLanguageIName = x.SecondLanguageIId != null ? x.SecondRoundResult.Name : "",
        ...
        public List<StudentsDto> GetAllGraduates(Guid UserId, Guid EmployeeId, Role RoleId)
        {
            using (var dbContext = new StudentAffairsEntities())
            {
                var model = dbContext.Students.Where(x => x.IsDeleted == false && x.StudentTypeId == (int)StudentTypes.Graduates && (x.CreatedBy == UserId || RoleId == Role.SystemAdmin)).OrderBy(x => x.CreatedOn).Select(x => new StudentsDto
                {
                    //بيانات الطالب
                    Id = x.Id,
                    Code = x.Code,
        ...
        public List<StudentsDto> GetAllTransferFromSchool(Guid UserId, Guid EmployeeId, Role RoleId)
        {
            using (var dbContext = new StudentAffairsEntities())
            {
                var model = dbContext.Students.Where(x => x.IsDeleted == false && x.StudentTypeId == (int)StudentTypes.TransferFromSchool && (x.CreatedBy == UserId || RoleId == Role.SystemAdmin)).OrderBy(x => x.CreatedOn).Select(x => new StudentsDto
                {
                    //بيانات الطالب
                    Id = x.Id,
                    Code = x.Code,
        ...
        public Student GetByCodeOrNameOrNumberId(string code)
        {
            using (var dbContext = new StudentAffairsEntities())
            {
                var model = dbContext.Students.Where(x => x.IsDeleted == false && (x.Code == code||x.Name.Contains(code)||x.NumberId==code)).OrderBy(x => x.CreatedOn).FirstOrDefault();
                return model;
            }
        }
        ...
        public ResultDto<Student> Create(Student model, Guid UserId)
        {
            using (var dbContext = new StudentAffairsEntities())
            {
                ...
                var Oldmodel = dbContext.Students.Where(x => (x.Name == model.Name ||x.NumberId==model.NumberId)&& x.IsDeleted == false).FirstOrDefault();
                if (Oldmodel != null)
                {
            ...
            using (var dbContext = new StudentAffairsEntities())
            {
                ...
                Oldmodel.Name = model.Name;
                   ...
                   Oldmodel.SeatNumber = model.SeatNumber;
                    ...
                    Oldmodel.SecondLanguageIId = model.SecondLanguageIId ;
