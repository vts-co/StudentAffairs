...
namespace StudentAffairs.Services.ExemptionReasons
{
    ...
    {
        public List<ExemptionReason> GetAll()
        {
            using (var dbContext = new StudentAffairsEntities())
            {
                var model = dbContext.ExemptionReasons.Where(x => x.IsDeleted == false).OrderBy(x => x.CreatedOn).ToList();
                return model;
            }
        }
        ...
        public ResultDto<ExemptionReason> Create(ExemptionReason model, Guid UserId)
        {
            using (var dbContext = new StudentAffairsEntities())
            {
                ...
                var Oldmodel = dbContext.ExemptionReasons.Where(x => x.Name == model.Name && x.IsDeleted == false).FirstOrDefault();
                if (Oldmodel != null)
                {
        ...
        public ResultDto<ExemptionReason> Edit(Level model, Guid UserId)
        {
            using (var dbContext = new StudentAffairsEntities())
            {
                var result = new ResultDto<ExemptionReason>();
